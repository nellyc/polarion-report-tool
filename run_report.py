...
for curr in field_dicts_list:
    g.update_sheet(row_number,5, find_number_of_TCs_per_status_and_run_fields(run_fields_dict=field_dicts[curr], status='*'))
    g.update_sheet(row_number,6, find_number_of_TCs_per_status_and_run_fields(run_fields_dict=field_dicts[curr], status='passed'))
    g.update_sheet(row_number,7, find_number_of_TCs_per_status_and_run_fields(run_fields_dict=field_dicts[curr], status='@null'))
    row_number += 1
